...
async function summarizationMemoryDemo() {
  ...
  if (allMessages.length > maxMessages) {
    ...
    const messagesToDelete = allMessages.slice(0, -keepRecent);

    console.log("\n💡 历史消息过多，开始总结...");
    console.log(`📝 将被总结的消息数量: ${messagesToDelete.length}`);
    console.log(`📝 将被保留的消息数量: ${recentMessages.length}`);

    const summary = await summarizeHistory(messagesToDelete);

    // 总结完之后,之前的内容可以删除,然后把总结加上剩下的信息传入
    await history.clear();
...
async function summarizeHistory(messages) {
  ...
  //   使用 getBufferString 来用于总结对话;
  const conversationText = getBufferString(messages, {
    humanPrefix: "用户",
    aiPrefix: "AI",
  });

  const summaryMessage = `请给我总结以下对话的核心内容,保留最重要的信息: ${conversationText}`;

  const summaryResponse = await model.invoke([
    new SystemMessage(summaryMessage),
  ]);
