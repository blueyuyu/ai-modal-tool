...
回复要简洁，只说做了什么`),
  ...
  // 对话人的对话存入信息中
  history.addMessage(new HumanMessage(query));
  ...
  for (let i = 0; i < maxIterations; i++) {
    ...
    for await (const chunk of rawStream) {
      ...
      if (parsedTools && parsedTools.length > 0) {
        console.log(
          chalk.bgYellow(`\n🔧 解析到工具调用: ${parsedTools[0].name}\n`),
        );
        console.log(
          chalk.yellow(
            `工具调用参数: ${JSON.stringify(parsedTools[0].args, null, 2)}\n`,
          ),
        );
        console.log("完整parsedTools:", JSON.stringify(parsedTools, null, 2));
    ...
    const finalMessage = history.getMessages();
